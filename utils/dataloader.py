...
def _resolve_pannuke_file(data_root, fold, filename):
    ...
    Supported layouts:
    - data_root/images/fold{fold}/images.npy
    - data_root/masks/fold{fold}/masks.npy
    - data_root/fold_{fold}/Fold {fold}/images/fold{fold}/images.npy
    - data_root/fold_{fold}/Fold {fold}/masks/fold{fold}/masks.npy
    """

    legacy_root = os.path.join(data_root, f'images/fold{fold}/images.npy') if filename == 'images.npy' else os.path.join(data_root, f'masks/fold{fold}/masks.npy')
    kaggle_root = os.path.join(
        data_root,
        f'fold_{fold}',
        f'Fold {fold}',
        'images' if filename == 'images.npy' else 'masks',
        f'fold{fold}',
        filename,
    )
    ...
    raise FileNotFoundError(
        'Could not resolve PanNuke file `%s` for fold %d from `%s`' % (filename, fold, data_root)
    )
...
class PannukeDataset(data.Dataset):
    ...
    def load_pannuke(self, data_root,fold=1):
        out_labels = []
        out_masks = []
        img_path = _resolve_pannuke_file(data_root, fold, 'images.npy')
        mask_path = _resolve_pannuke_file(data_root, fold, 'masks.npy')
        out_imgs=np.load(img_path).astype(np.uint8)#(2523, 256, 256, 3)
        masks=np.load(mask_path).astype(np.int16)#(2523, 256, 256, 6)
